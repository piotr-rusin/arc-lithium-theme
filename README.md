...
After the installation is complete the theme can be activated with `gnome-tweak-tool` or a similar program by selecting `Arc-Lithium`, `Arc-Lithium-Darker` or `Arc-Lithium-Dark` as Window/GTK+ theme and `Arc-Lithium` or `Arc-Lithium-Dark` as GNOME Shell/Cinnamon theme.
